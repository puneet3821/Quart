		...
		if($type == "question"){
				...
				while($row = mysql_fetch_array($result)){
					...
					if (($key = array_search($user_id, $downvoteList)) !== false) {
						...
						if (($key = array_search($user_id, $upvoteList)) !== false) {
	    					...
	    					$upvotes -= 1;
	    					array_push($downvoteList, $user_id);
							$downvotes += 1; 
			...
			if($result){
				while($row = mysql_fetch_array($result)){
					...
					$query = "UPDATE `quart_answers` SET `upvote`='$upvotes',`downvote`='$downvotes', `upvotedBy`='$upvotedBy' , `downvotedBy`='$downvotedBy' WHERE `answer_id` = '$id'";
					$res = mysql_query($query);
					if($res){
					}
					else{
